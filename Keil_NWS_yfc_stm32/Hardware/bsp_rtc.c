...
/************************************************************************
 @brief  NVIC_Configuration(配置RTC秒中断的主中断优先级为1，次优先级为0)
 @param  无
 @retval 无
************************************************************************/
void RTC_NVIC_Config(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = RTC_IRQn;//中断通道选择RTC
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//抢占优先级设为1
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;//从优先级设为0
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//命令使能
	NVIC_Init(&NVIC_InitStructure);
}


/************************************************************************
 @brief  RTC_Configuration(配置RTC)
		 LSE:外部 32.768 kHz
		 LSI:内部 40 kHz
 @param  无
 @retval 无
...
void RTC_Configuration(void)
{
	...
	/* 允许访问 Backup 区域 */
	//DBP bit of PWR_CR register
	PWR_BackupAccessCmd(ENABLE);
	
	/* 复位 Backup 区域 */
	//BDRST bit of RCC_BDCR register
	BKP_DeInit();
...
//批量产品的时候，很容易出现外部晶振不起振的情况，不太可靠	
	...
	/* 使能 LSI */
	//LSION bit of RCC_CSR register
	RCC_LSICmd(ENABLE);

	/* 等待 LSI 准备好 */
	//LSIRDY bit of RCC_CSR register
	while (RCC_GetFlagStatus(RCC_FLAG_LSIRDY) == RESET);
	
	/* 选择 LSI 作为 RTC 时钟源 */
	//RTCSEL[1:0] bits of RCC_BDCR register
	RCC_RTCCLKConfig(RCC_RTCCLKSource_LSI);
	
	/* 使能 RTC 时钟 */
	//RTCEN bit of RCC_BDCR register
	RCC_RTCCLKCmd(ENABLE);
	
	/* 等待 RTC 寄存器 同步
	 * 因为RTC时钟是低速的，内环时钟是高速的，所以要同步
	 */
	//RSF bit of RTC_CRL register 
	RTC_WaitForSynchro();
	
	/* 确保上一次 RTC 的操作完成 */
	//RTOFF bit of RTC_CRL register
	RTC_WaitForLastTask();
	
	/* 使能 RTC 秒中断 */
	//SECIE bit of RTC_CRH register
	RTC_ITConfig(RTC_IT_SEC, ENABLE);
...
/************************************************************************
 @brief  Time_Adjust(时间调节)
 @param  用于读取RTC时间的结构体指针（北京时间）
 @retval 无
************************************************************************/
...
/************************************************************************
 @brief  RTC_CheckAndConfig(检查并配置RTC)
 @param  用于读取RTC时间的结构体指针
 @retval 无
************************************************************************/
...
/************************************************************************
 @brief  Time_Regulate_Get
	     保存用户使用串口设置的时间，
         以便后面转化成时间戳存储到RTC 计数寄存器中。
 @param  用于读取RTC时间的结构体指针,在串口调试助手输入时，输入完数字要加回车
 @retval 无
************************************************************************/
